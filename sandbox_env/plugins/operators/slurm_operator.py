...
class SubmitAndMonitorSlurmJobOperator(BaseOperator):
    def __init__(self, ssh_conn_id, script_path, qos, name, poll_interval=60, **kwargs):
    ...
    def execute(self, context):
        ...
        with hook.get_conn() as ssh_client:
            stdin, stdout, stderr = ssh_client.exec_command(f"sbatch --qos={self.qos} --job-name={self.name}  {self.script_path}")
